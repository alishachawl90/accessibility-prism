...
export function renderSrWalkthrough(data: WalkthroughData): string {
  let html = renderNavBar('Screen Reader Walk-Through', true, 'Back');

  const total = data.entries.length;
  const current = data.currentIndex;

  html += `
    <div style="padding: 12px 16px !important; background: white !important; border-bottom: 1px solid ${BORDER} !important;">
      <div style="display: flex !important; align-items: center !important; justify-content: space-between !important; margin-bottom: 8px !important;">
        ...
        <div style="display: flex !important; gap: 6px !important;">
          <button id="btn-wt-prev" ${current <= 0 ? 'disabled' : ''} style="padding: 4px 12px !important; border: 1px solid #D1D5DB !important; border-radius: 6px !important; font-size: 12px !important; cursor: ${current <= 0 ? 'not-allowed' : 'pointer'} !important; background: white !important; color: ${current <= 0 ? '#6B7280' : '#374151'} !important; font-weight: 600 !important;">&#9664; Prev</button>
          <button id="btn-wt-next" ${current >= total - 1 ? 'disabled' : ''} style="padding: 4px 12px !important; border: 1px solid #D1D5DB !important; border-radius: 6px !important; font-size: 12px !important; cursor: ${current >= total - 1 ? 'not-allowed' : 'pointer'} !important; background: white !important; color: ${current >= total - 1 ? '#6B7280' : '#374151'} !important; font-weight: 600 !important;">Next &#9654;</button>
        </div>
      </div>
      <div style="height: 4px !important; background: #E5E7EB !important; border-radius: 2px !important; overflow: hidden !important;">
        <div style="height: 100% !important; width: ${total > 0 ? ((current + 1) / total) * 100 : 0}% !important; background: #2563EB !important; border-radius: 2px !important; transition: width 0.2s !important;"></div>
      </div>
    </div>
  `;

  if (total === 0) {
    html += `<div style="padding: 24px !important; text-align: center !important; color: #6B7280 !important; font-size: 14px !important;">No significant elements found.</div>`;
    return html;
  }
  ...
  html += `
    <div id="scroll-area" style="padding: 10px 16px !important; background: #F9FAFB !important; overflow-y: auto !important; flex: 1 !important;">
...
export function attachWalkthroughListeners(container: HTMLElement, data: WalkthroughData, actions: {
  onBack: () => void;
  onHighlight: (els: Element[]) => void;
  onNavigate: (index: number) => void;
}): void {
  container.querySelector('#btn-back')?.addEventListener('click', () => actions.onBack());
  container.querySelector('#btn-wt-prev')?.addEventListener('click', () => {
    if (data.currentIndex > 0) actions.onNavigate(data.currentIndex - 1);
  });
  container.querySelector('#btn-wt-next')?.addEventListener('click', () => {
    if (data.currentIndex < data.entries.length - 1) actions.onNavigate(data.currentIndex + 1);
  });
